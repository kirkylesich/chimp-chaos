...
impl MeshAdapter for NoopMesh {
    async fn apply_fault(&self, _spec: &domain::models::ChaosExperimentSpec) -> Result<(), DomainError> { Ok(()) }
    async fn rollback_fault(&self, _spec: &domain::models::ChaosExperimentSpec) -> Result<(), DomainError> { Ok(()) }
}
...
impl MetricsBackend for NoopMetrics {
    async fn fetch_value(&self, _query: &str, _window: &str) -> Result<f64, DomainError> { Ok(1.0) }
}
...
impl TraceBackend for NoopTrace {
    async fn fetch_edges(&self, _window: &str) -> Result<Vec<domain::models::Edge>, DomainError> { Ok(Vec::new()) }
}
...
impl ReportSink for NoopSink {
    async fn store_report(&self, _report: &domain::models::ChaosReport) -> Result<(), DomainError> { Ok(()) }
}
